...
foreach($comments as $comment) :
	...
	$tooltip_inline = $check2['tooltip_inline'];
			...
			if($tooltip_inline == 1){
				$aggr_show = 'title="Average rating is '.round($result, 2).' out of 5"';
				$addclass= 'wpcr_tooltip';
			}else{
				$aggr_showin = ''.round($result, 2).' out of 5';
				$addclass= 'wpcr_inline';
			}
			...
			if($check_val == 'yes'){
				if($count != 0){ 
					...
					$output ='<div class="wpcr_aggregate"><a class="'.$addclass.'" '.$aggr_show.'><span class="wpcr_stars" title="">Average rating:</span>';
					$output .= '<span class="wpcr_averageStars" id="'.$result.'"></span></a> '.$aggr_showin.'';
					$output .= '</div>';
				} 
